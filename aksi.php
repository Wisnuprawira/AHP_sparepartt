...
//     $type = $_POST['type'];
    ...
    // Cek jika semua data ada dan valid
    // if ($kode_kriteria && $nama_kriteria && is_numeric($bobot) && ($type == 'benefit' || $type == 'cost')) {
        
    //     // Cek apakah kode_kriteria sudah ada di database
    //     $check = $db->query("SELECT * FROM tb_kriteria_saw WHERE kode_kriteria = '$kode_kriteria'");
        
    //     if ($check->num_rows > 0) {
    //         // Jika kode_kriteria sudah ada, simpan pesan error di session
    //         $_SESSION['error'] = "Kode kriteria telah digunakan!";
    //     } else {
    //         // Jika kode_kriteria belum ada, lakukan insert data
    //         $db->query("INSERT INTO tb_kriteria_saw (kode_kriteria, nama_kriteria, bobot, type) VALUES ('$kode_kriteria', '$nama_kriteria', $bobot, '$type')");
    //         unset($_SESSION['error']); // hapus pesan error jika berhasil
    //         header("location:index.php?m=kriteria_saw");
    //         exit;
    //     }
    // } else {
    //     $_SESSION['error'] = "Data tidak lengkap atau tidak valid!";
    // }
    // header("location:index.php?m=kriteria_saw_tambah"); // kembali ke halaman tambah
    // exit;
// }

/** Fungsi Ubah Kriteria SAW **/
// elseif ($action == 'ubah' && $_SERVER['REQUEST_METHOD'] == 'POST') {
//     $kode_kriteria = $_GET['kode'];
//     $nama_kriteria = $_POST['nama_kriteria'];
//     $bobot = $_POST['bobot'];
//     $type = $_POST['type'];

//     echo "Updating kriteria: $kode_kriteria, $nama_kriteria, $bobot, $type<br>"; // Debug checkpoint

//     if ($kode_kriteria && $nama_kriteria && is_numeric($bobot) && ($type == 'benefit' || $type == 'cost')) {
//         $db->query("UPDATE tb_kriteria_saw SET nama_kriteria = '$nama_kriteria', bobot = $bobot, type = '$type' WHERE kode_kriteria = '$kode_kriteria'");
//         header("location:index.php?m=kriteria_saw");
//         exit;
//     } else {
...
/** Fungsi Hapus Kriteria SAW **/
// if ($action == 'hapus') {
//     // Ambil parameter kode
//     $kode_kriteria = isset($_GET['kode']) ? $_GET['kode'] : '';

//     // Debug: Pastikan action dan kode diakses
//     echo "Fungsi hapus diakses. Action: $action, Kode Kriteria: $kode_kriteria<br>";

//     // Pastikan kode tidak kosong
//     if (!empty($kode_kriteria)) {
//         // Lakukan query hapus
//         $delete = $db->query("DELETE FROM tb_kriteria_saw WHERE kode_kriteria = '$kode_kriteria'");
        
//         if ($delete) {
//             // Jika berhasil, kembali ke halaman kriteria_saw
//             header("Location: index.php?m=kriteria_saw");
//             exit;
//         } else {
...
//         }
//     } else {
//         echo "Kode kriteria tidak ditemukan atau tidak valid!";
//     }
//     exit;
// }
?>